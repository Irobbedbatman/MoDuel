...
public static class PackagedCodeFinder {
    ...
    public static PackagedCode FindAndInit(Package sourcePackage) {
        ...
        Type? type = null;
        // Check to see if the type should be acquired through it's parameters.
        if (sourcePackage.GetProperty<string>(PACKAGE_CLASS_PROPERTY) != null)
            type = GetEntryFromParameters(assembly, sourcePackage);
    ...
    /// <para>Performs checks to ensure the most accurate <see cref="PackagedCode"/> is returned.</para>
    /// <para>The behaviour of this method is directly affected by the <see cref="PackagedCodeBindingAttribute"/>.</para>
    /// </summary>
    /// <param name="assembly">The assembly to search.</param>
    /// <param name="packageName">The package name property that can be passed the entry attribute to check for matches.</param>
    ...
    public static Type? GetEntryThroughReflection(Assembly assembly, string? packageName = null) {
        ...
        // Check to see if the packageName is equivalent
        if (packageName == null)
            return null;
    ...
    /// </summary>
    /// <returns>The found type that matches the properties provided. Null if it couldn't be found or if the type was not a <see cref="PackagedCode"/>..</returns>
    /// <exception cref="TypeLoadException"></exception>
    public static Type? GetEntryFromParameters(Assembly assembly, Package package) {
        ...
        // Add the namespace separator if a namespace exists.
        ...
        LogSettings.LogEvent($"Failed to load the packaged code type from {string.Concat(typeNamespace, typeClassName ?? package.Name)}", LogSettings.LogEvents.DataLoadingError);
