...
/// <summary>
/// Objects that can explicitly be told to react to a certain trigger keyword.
/// </summary>
public interface IExplicitTriggerable {

    /// <summary>
    /// Get the response the object will have when it is informed to react to the provided <paramref name="trigger"/>.
    /// </summary>
    ...
    /// <param name="fallback">The action to call if no explicit trigger for the provided <paramref name="trigger"/> could be found.</param>
    /// <param name="arguments">The arguments that will be passed to the reaction alongside this.</param>
    /// <returns>The result of the reaction to the <paramref name="trigger"/> or the result of <paramref name="fallback"/> if no reaction exists. </returns>
